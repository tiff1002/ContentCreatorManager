...
class Twitter(contentcreatormanager.platform.platform.Platform):
    ...
    def api_update_status(self, status_text : str, attachment_url : str = '', media_ids : list = [], possibly_sensitive : bool = False):
        """
        ...
        Possible Exceptions: tweepy.errors.BadRequest: 400 Bad Request (This is if you provide an invalid attachment_url as this must be a Tweet permalink or Direct Message deep link)
        """
        self.logger.info("Making update_status Twitter API Call")
        
        if attachment_url == '' or attachment_url is None:
            return self.api.update_status(status=status_text, media_ids=media_ids, possibly_sensitive=possibly_sensitive)
        else:
            return self.api.update_status(status=status_text, media_ids=media_ids, possibly_sensitive=possibly_sensitive, attachment_url=attachment_url)