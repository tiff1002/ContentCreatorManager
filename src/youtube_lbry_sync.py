...
youtube = contentcreatormanager.platform.youtube.YouTube(settings=settings, init_videos=True, current_quota_usage=0)
...
for v in lbry_not_youtube:
    yvid = contentcreatormanager.media.video.youtube.YouTubeVideo(channel=youtube, self_declared_made_for_kids=False, made_for_kids=False, public_stats_viewable=True, embeddable=True, privacy_status='unlisted', title=v.title, description=v.description, file_name=os.path.basename(v.file), update_from_web=False, tags=v.tags, new_video=True)
    youtube.add_media(yvid)
    yvid.upload()

for v in youtube_not_lbry:
    lvid = contentcreatormanager.media.video.lbry.LBRYVideo(lbry_channel=lbry, tags=v.tags, title=v.title, file_name=os.path.basename(v.file), description=v.description)
    lbry.add_media(lvid)
    lvid.upload()
